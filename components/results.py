...
from config import get_default_stress_events


def _build_positions_df(trades_df: pd.DataFrame, portfolio_df: pd.DataFrame) -> Optional[pd.DataFrame]:
    ...
    shares_col = "shares" if "shares" in trades_df.columns else "amount" if "amount" in trades_df.columns else None
    price_col = "close" if "close" in trades_df.columns else "fill_price" if "fill_price" in trades_df.columns else None
    rows = []
...
def render_backtest_results(result, config: Optional[dict] = None):
    ...
    (tab_equity, tab_drawdown, tab_performance,
     tab_risk, tab_rollover, tab_trades,
     tab_execution, tab_monthly, tab_yearly, tab_regime) = tabs
    ...
    with tab:
        if portfolio_df is not None and not portfolio_df.empty:
            with st.expander("滚动 VaR（历史模拟法）"):
                ...
                eq_col = "equity" if "equity" in portfolio_df.columns else portfolio_df.columns[0]
                if "date" in portfolio_df.columns:
                    daily_ret = portfolio_df.set_index("date")[eq_col].pct_change().dropna()
                else:
                    daily_ret = portfolio_df[eq_col].pct_change().dropna()
                fig_var = PlotManager.plot_rolling_var(daily_ret, window=var_window, ci=var_ci)
            ...
            with st.expander("多品种相关性热力图"):
                if trades_df is not None and not trades_df.empty and "symbol" in trades_df.columns:
                    ...
                    if env_df is not None and not env_df.empty:
                        pivot_ret = env_df[env_df["symbol"].isin(symbols_in_trades)].pivot_table(
            ...
            with st.expander("上下捕获比率"):
                if trades_df is not None and not trades_df.empty:
                    eq_col = "equity" if "equity" in portfolio_df.columns else portfolio_df.columns[0]
                    if "date" in portfolio_df.columns:
                        strat_ret = portfolio_df.set_index("date")[eq_col].pct_change().dropna()
                    else:
                        strat_ret = portfolio_df[eq_col].pct_change().dropna()
                    env_df = st.session_state.get("env_df")
                    if env_df is not None and "close" in env_df.columns:
                        dom = env_df[safe_col(env_df, "is_dominant") if "is_dominant" in env_df.columns else pd.Series([True] * len(env_df))].copy()
                        if not dom.empty:
            ...
            with st.expander("风险贡献饼图"):
                if trades_df is not None and not trades_df.empty and "symbol" in trades_df.columns:
                    ...
                    if pnl_col in trades_df.columns:
                        risk_contrib = trades_df.groupby("symbol")[pnl_col].std().to_dict()
            ...
            with st.expander("持仓集中度曲线"):
                if trades_df is not None and not trades_df.empty and "symbol" in trades_df.columns:
                    ...
                    if positions_df is not None and not positions_df.empty:
                        fig_conc = PlotManager.plot_concentration_curve(portfolio_df, positions_df)
            ...
            with st.expander("杠杆率曲线"):
                if trades_df is not None and not trades_df.empty and "symbol" in trades_df.columns:
                    positions_df = _build_positions_df(trades_df, portfolio_df)
                    fig_lev = PlotManager.plot_leverage_ratio(portfolio_df, positions_df)
            ...
            if not rollover_dates.empty:
                with st.expander("展期时间线", expanded=True):
                    ...
                    if price_df is not None and "is_dominant" in price_df.columns:
                        dom_price = price_df[safe_col(price_df, "is_dominant")][["date", "close"]]
                    else:
                        dom_price = None
                    fig_roll = PlotManager.plot_rollover_timeline(
                        rollover_dates,
                        price_df=dom_price if dom_price is not None and not dom_price.empty else None,
                    )
    ...
    with tab:
        if trades_df is not None and not trades_df.empty:
            ...
            if config.get("save_trades", True):
                ...
                st.download_button(
                    "📥 下载交易记录 CSV", csv_data, "trades.csv", "text/csv",
                )
            ...
            if config.get("save_trades", True):
                ...
                st.download_button(
                    "📥 下载订单记录 CSV", csv_data, "orders.csv", "text/csv",
                )
        else:
            st.info("无交易记录")
    ...
    with tab:
        if orders_df is not None and not orders_df.empty:
            ...
            with st.expander("滑点时间序列"):
                cumulative_slip = st.checkbox("显示累积滑点", value=False, key="cum_slip")
                fig_slip = PlotManager.plot_slippage_time(orders_df, cumulative=cumulative_slip)
    ...
    with tab:
        if portfolio_df is not None and not portfolio_df.empty:
            ...
            **逐年收益率分析**帮助您了解策略在不同年份的表现差异：
            ...
            eq_col = "equity" if "equity" in portfolio_df.columns else portfolio_df.columns[0]
            date_col = "date" if "date" in portfolio_df.columns else portfolio_df.columns[0]
            ...
            if date_col in df_yr.columns:
                ...
                with st.expander("逐年收益率柱状图", expanded=True):
                    ...
                    fig_yr.add_hline(
                        y=mean_return, line_dash="dash", line_color="#1f77b4",
                        line_width=1.5,
                    ...
                    for i, count in enumerate(consecutive_loss):
                        if count >= 2:
                            fig_yr.add_vrect(
                                ...
                                fillcolor="rgba(214, 39, 40, 0.1)", line_width=0
                            )
                ...
                with st.expander("逐年收益率数据表", expanded=True):
                    ...
                    with col2:
                        st.metric("盈利年数", f"{profit_years}", delta=f"{profit_years/total_years*100:.0f}%")
                    with col3:
                        st.metric("亏损年数", f"{loss_years}", delta=f"-{loss_years/total_years*100:.0f}%")
                    with col4:
                        best_idx = yearly_eq["return_pct"].idxmax()
                        st.metric("最佳年份", f"{int(yearly_eq.loc[best_idx, 'year'])}年",
                                   delta=f"{yearly_eq.loc[best_idx, 'return_pct']:.2f}%")
                    with col5:
                        worst_idx = yearly_eq["return_pct"].idxmin()
                        st.metric("最差年份", f"{int(yearly_eq.loc[worst_idx, 'year'])}年",
                    ...
                    st.dataframe(
                        display_df.style.applymap(
                            ...
                            else "background-color: #f8d7da; color: #721c24" if isinstance(v, (int, float)) and v < 0
                            else "",
                            subset=["收益率(%)"]
                        ),
                    ...
                    st.download_button(
                        "📥 下载逐年收益率 CSV", csv_data,
                        "yearly_returns.csv", "text/csv"
                    )
...
def _render_failure_analysis(years, returns):
    ...
    if current >= 2:
        failure_periods.append({
            ...
            "结束年份": years[-1],
            "持续年数": current,
            "累计亏损": sum(returns[j] for j in range(len(returns) - current, len(returns)))
        })
    ...
    if failure_periods:
        ...
        st.warning("⚠️ 连续亏损区间提示：策略在这些时段可能不适应市场环境，"
...
def _render_regime_tab(tab, portfolio_df):
    with tab:
        ...
        if env_df is not None and "env_market_regime" in env_df.columns:
            dominant_data = env_df[safe_col(env_df, "is_dominant") if "is_dominant" in env_df.columns else pd.Series([True] * len(env_df))].copy()
            if not dominant_data.empty:
                ...
                with st.expander("各市场状态绩效"):
                    ...
                    fig_reg_perf = PlotManager.plot_regime_performance(
                        portfolio_df, dominant_data.set_index("date")[regime_s.name]
                        if "date" in dominant_data.columns else regime_s
                    )
                    st.plotly_chart(fig_reg_perf, use_container_width=True)

                with st.expander("价格 + 买卖信号标记"):
                    if "buy" in dominant_data.columns or "sell" in dominant_data.columns:
