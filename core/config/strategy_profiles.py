...
# 2026-06-11 重构：vol_breakout 重写因子（V_04 OI 跳变 + H_05 加权和）后回归 5 子策略
    ...
    def _init_default_library(self):
        ...
        self.register(
            StrategyProfile(
                ...
                "【2026-06-11 重写】V_04 加 OI 跳变检测、H_05 改加权和消除三重共振失效、"
                "ma_window 网格从 [3,5,7,10,14] 改为 [10,15,20,30]，corr_window 改为 [40,60,90,120]。",
                default_params={
                    "ma_window": 10,
                    "corr_window": 60,
                    "position_size": 0.2,
                ...
                param_ranges={
                    # 2026-06-11 重构：删除过拟合值 3 / 120，加入稳健值
                    "ma_window": [10, 15, 20, 30],
                    "corr_window": [40, 60, 90, 120],
                },
                factor_weights={"vol_breakout": 0.2},
                max_position_pct=0.2,
                stop_loss_pct=0.04,
                enabled=True,  # 2026-06-11 重构：重写因子后恢复启用
            )
        )
        self.register(
            StrategyProfile(
        ...
        self.register(
            StrategyProfile(
                ...
                factor_weights={
                    # 2026-06-11 重构：5 子策略等权 0.2（vol_breakout 重写后回归）
                    "trend": 0.20,
                    "term_structure": 0.20,
                    "mean_reversion": 0.20,
                    "vol_breakout": 0.20,
