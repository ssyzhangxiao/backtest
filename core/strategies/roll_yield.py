...
class RollYieldStrategy(BaseStrategy):
    ...
    def register_indicators(self) -> List:
        """注册展期收益指标：价格偏离长期均值的百分比。"""
        lookback = self.lookback

        def calc_ma(data):
            return pd.Series(data.close).rolling(window=lookback, min_periods=lookback).mean().values

        ma_indicator = pybroker.indicator(self._ma_name, calc_ma)
        return [ma_indicator]
    ...
    def execute(self, ctx: ExecContext) -> None:
        ...
        try:
            ma_arr = ctx.indicator(self._ma_name)
        except (ValueError, KeyError):
            return
        if ma_arr is None or len(ma_arr) == 0:
            return

        ma_val = float(ma_arr[-1])
        if np.isnan(ma_val) or ma_val == 0:
            return

        current_price = float(ctx.close[-1])
        spread_pct = (current_price - ma_val) / ma_val * 100
        ...
        if long_pos:
            if spread_pct > -self.exit_threshold:
