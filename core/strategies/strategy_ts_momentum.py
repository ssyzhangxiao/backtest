...
因子计算委托 core/factors/basic_factors.py:compute_ts_momentum()
"""
...
from core.factors import compute_ts_momentum
...
class TSMomentumStrategy(BaseStrategy):
    ...
    def register_indicators(self) -> List:
        """注册动量指标：N日累计收益率（委托 core.factors.compute_ts_momentum）。"""
        window = self.window

        def calc_momentum(data):
            close_series = pd.Series(data.close)
            return compute_ts_momentum(close_series, window).values
